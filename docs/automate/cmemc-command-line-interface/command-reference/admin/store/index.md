...
!!! note
    There is currently no deletion mechanism for the bootstrap data and you need to remove the respective graphs manually (or just remove all graphs).
...
The backup file is a ZIP archive containing all knowledge graphs as Turtle files + configuration files for each graph.
...
This command will create lots of load on the server. It can take a long time to complete. The backup file will be transferred to the server, then unzipped and imported graph by graph. After the initial transfer, the network connection is not used anymore, so it will sometimes be closed by proxies. This does not mean that the import failed.
